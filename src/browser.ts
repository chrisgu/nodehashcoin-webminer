type Preset = "desktop" | "laptop" | "phone";

const PRESETS: Record<Preset, { duty: number; label: string }> = {
  desktop: { duty: 0.75, label: "Desktop · ~75% CPU" },
  laptop: { duty: 0.4, label: "Laptop · ~40% CPU" },
  phone: { duty: 0.15, label: "Phone · ~15% CPU" },
};

type Work = { height: number; block: { header: Record<string, unknown>; txs: unknown[] } };

const qs = new URLSearchParams(location.search);
const nodeUrl = (qs.get("node") || (window as unknown as { NHC_NODE?: string }).NHC_NODE || "/api").replace(/\/$/, "");
const workerUrl = new URL("./miner-worker.js", import.meta.url).href;

let workers: Worker[] = [];
let hashAcc = 0;
let running = false;

function el<T extends HTMLElement>(id: string): T {
  const n = document.getElementById(id);
  if (!n) throw new Error(`#${id}`);
  return n as T;
}

function setStatus(s: string): void {
  el("status").textContent = s;
}

function persistAddress(addr: string): void {
  localStorage.setItem("nhc.payout", addr);
}

function loadAddress(): string {
  return localStorage.getItem("nhc.payout") || "";
}

async function fetchWork(payout: string): Promise<Work> {
  const res = await fetch(`${nodeUrl}/work?payout=${encodeURIComponent(payout)}`);
  const body = await res.json();
  if (!res.ok) throw new Error(body.error || `work ${res.status}`);
  return body as Work;
}

async function submit(block: unknown): Promise<void> {
  const res = await fetch(`${nodeUrl}/submit`, {
    method: "POST",
    headers: { "content-type": "application/json" },
    body: JSON.stringify({ block }),
  });
  const body = await res.json();
  if (!res.ok) throw new Error(body.error || `submit ${res.status}`);
  setStatus(`Accepted height ${body.height}`);
}

function stop(): void {
  running = false;
  for (const w of workers) {
    w.postMessage({ type: "stop" });
    w.terminate();
  }
  workers = [];
  el<HTMLButtonElement>("start").disabled = false;
  el<HTMLButtonElement>("stop").disabled = true;
}

async function start(): Promise<void> {
  const payout = el<HTMLInputElement>("payout").value.trim();
  if (!/^nhc1[0-9a-f]{40}$/.test(payout)) {
    setStatus("Enter a valid nhc1… payout address (create one in the wallet).");
    return;
  }
  persistAddress(payout);
  const preset = el<HTMLSelectElement>("preset").value as Preset;
  const duty = PRESETS[preset].duty;
  const cores = Math.max(1, Math.min(navigator.hardwareConcurrency || 2, 8));
  const n = Math.max(1, Math.round(cores * duty));

  running = true;
  el<HTMLButtonElement>("start").disabled = true;
  el<HTMLButtonElement>("stop").disabled = false;
  hashAcc = 0;

  const tick = () => {
    el("rate").textContent = `${(hashAcc / 1_000_000).toFixed(2)} MH/s`;
    hashAcc = 0;
    if (running) setTimeout(tick, 1000);
  };
  tick();

  while (running) {
    try {
      setStatus(`Fetching work…`);
      const work = await fetchWork(payout);
      setStatus(`Mining height ${work.height} (BLAKE3)`);
      const found = await mineOne(work, n, duty);
      if (!found || !running) break;
      await submit({ header: found.header, txs: work.block.txs });
    } catch (err) {
      setStatus(err instanceof Error ? err.message : "miner error");
      await new Promise((r) => setTimeout(r, 2500));
    }
  }
}

function mineOne(work: Work, n: number, duty: number): Promise<{ header: Record<string, unknown>; hash: string } | null> {
  return new Promise((resolve) => {
    let settled = false;
    const finish = (v: { header: Record<string, unknown>; hash: string } | null) => {
      if (settled) return;
      settled = true;
      for (const w of workers) {
        w.postMessage({ type: "stop" });
        w.terminate();
      }
      workers = [];
      resolve(v);
    };
    for (let i = 0; i < n; i++) {
      const w = new Worker(workerUrl, { type: "module" });
      w.onmessage = (ev: MessageEvent<{ type: string; hashes?: number; header?: Record<string, unknown>; hash?: string }>) => {
        if (ev.data.type === "stat") hashAcc += ev.data.hashes || 0;
        if (ev.data.type === "found" && ev.data.header && ev.data.hash) {
          finish({ header: ev.data.header, hash: ev.data.hash });
        }
      };
      w.postMessage({ type: "start", header: work.block.header, duty, workerId: i });
      workers.push(w);
    }
  });
}

function boot(): void {
  el<HTMLInputElement>("payout").value = loadAddress();
  el<HTMLButtonElement>("start").addEventListener("click", () => void start());
  el<HTMLButtonElement>("stop").addEventListener("click", () => {
    stop();
    setStatus("Stopped.");
  });
  el<HTMLSelectElement>("preset").addEventListener("change", () => {
    if (running) {
      stop();
      void start();
    }
  });
}

boot();
